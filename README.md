...
- **Track history**: the **Track history** tab lists what your favorites played in the last 12 hours, whether or not you were listening, so "what was that song four minutes ago?" has an answer. Searchable, and it survives a restart.
...
- **Settings** (the gear at the right of the tabs): start with Windows, language, per-favorite loudness, station list info and update check, clearing the logo and popularity cache, checking for a new version of the app, and the app version.
...
- **Shortcuts**: `Ctrl+Space` stop/play, `Ctrl+M` mute, `Ctrl+F` search. From any app: `Ctrl+Alt+P` stop/play, `Ctrl+Alt+M` mute, `Ctrl+Alt+→` / `Ctrl+Alt+←` next/previous favorite (these two also bring the window up, without taking focus). They are listed on the Zapper tab, where the global ones can be switched off.
...
After every new release, the [Store MSIX workflow](.github/workflows/store-msix.yml) builds unsigned x64 and ARM64 `.msixupload` files for that release, with version `<version>.0`, keeps them as workflow artifacts and submits them to the Store with `scripts/submit-store.ps1`. That script creates a new submission through the Partner Center submission API, swaps its packages for the new ones and commits it; the update is published as soon as it passes certification. It signs in as a Microsoft Entra app with the Manager role in Partner Center, whose details are the repository secrets `PARTNER_CENTER_TENANT_ID`, `PARTNER_CENTER_CLIENT_ID` and `PARTNER_CENTER_CLIENT_SECRET`; without them the packages are only built. The workflow can also be started by hand for a release tag, and then only submits when asked to. To try the Store version on your own PC:
