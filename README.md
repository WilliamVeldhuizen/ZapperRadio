...
- **Shortcuts**: `Ctrl+Space` stop/play, `Ctrl+M` mute, `Ctrl+F` search. From any app: `Ctrl+Alt+P` stop/play, `Ctrl+Alt+M` mute, `Ctrl+Alt+→` / `Ctrl+Alt+←` next/previous favorite (these two also bring the window up, without taking focus). The global ones can be switched off in the settings.
