...
- **The same shortcuts from any app**: `Ctrl+Alt+P` to stop or play, `Ctrl+Alt+M` to mute or unmute, and `Ctrl+Alt+Right` and `Ctrl+Alt+Left` for the next and previous favorite, also while you are working somewhere else. On `Ctrl+Alt` rather than `Ctrl`, because claiming `Ctrl+Space` system wide would take it away from every other app. A combination another app already holds is left to that app and named in the settings, where the whole set can also be switched off.
